/**
 * AuthorCard — author avatar (photo/initials), name (→ profile), summary, links, email.
 */
...
describe('AuthorCard', () => {
  it('renders name (→ profile), summary, links, and initials when no photo', async () => {
    ...
    // email shown as a mailto when the profile exposes one
    expect(w.find('[data-testid="author-card-email"] a').attributes('href')).toBe('mailto:author@shown.io')
    expect(w.find('[data-testid="author-card-email"]').text()).toContain('author@shown.io')
  ...
  it('omits summary/links when absent', async () => {
    ...
    expect(w.find('[data-testid="author-card-email"]').exists()).toBe(false)
  })
})
