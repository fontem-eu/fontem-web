/**
 * Tests for the stats API client (catalog + series).
 */
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest'
import { fetchDatasets, fetchSeries } from '../../src/api/stats.js'
...
describe('fetchDatasets', () => {
  it('hits /api/stats/datasets', async () => {
    globalThis.fetch.mockResolvedValue({ ok: true, json: async () => [] })
    await fetchDatasets()
    const url = globalThis.fetch.mock.calls[0][0]
    expect(url.startsWith('/api/stats/datasets')).toBe(true)
  })
...
describe('fetchSeries', () => {
  ...
  it('serialises start, end, dimensions', async () => {
    ...
    // dimensions is JSON-stringified then URL-encoded
