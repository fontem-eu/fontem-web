<script setup>
/**
 * Data Studio — home. Lists data projects (or an empty state) and creates new
 * ones. A project groups queries and plots.
 */
...
<template>
  <div class="home" data-testid="studio-home">
    ...
    <p v-if="!studio.projects.value.length" class="empty" data-testid="studio-empty">
    ...
    <ul v-else class="plist">
      <li v-for="p in studio.projects.value" :key="p.id">
        <router-link :to="`/studio/p/${p.id}`" class="pcard" data-testid="studio-project-card">
          ...
          <span class="pcard-meta">{{ p.queries.length }} {{ p.queries.length === 1 ? 'query' : 'queries' }}</span>
        </router-link>
      </li>
    </ul>
  </div>
