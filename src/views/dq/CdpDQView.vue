...
onMounted(() => { document.title = 'CDP Data Quality — GMR' })
const data = ref(null)
...
<template>
  <div class="dq"><header class="dq-hdr"><div><router-link to="/admin/data-quality" class="dq-back">&larr; Data Quality</router-link><h1>CDP Climate Disclosure</h1><p class="dq-sub">CDP scores and climate reporting — score distribution and reporting years</p></div><ThemeToggle /></header>
    ...
    <template v-else-if="data">
      ...
      <section class="dq-section"><h2>Score Distribution</h2><HorizontalBarChart :data="scoreBars" :max-bars="10" /></section>
      <section class="dq-section"><h2>By Reporting Year</h2><HorizontalBarChart :data="yearBars" :max-bars="10" /></section>
    </template>
  </div>
</template>
<style scoped>
