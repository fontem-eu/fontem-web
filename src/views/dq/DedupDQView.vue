...
onMounted(() => { document.title = 'Deduplication Data Quality — Dargle' })
const data = ref(null)
...
<template>
  <div class="dq"><header class="dq-hdr"><div><router-link to="/data-quality" class="dq-back">{{ $t('nav.back_data_quality') }}</router-link><h1>{{ $t('dedup_d_q.deduplication') }}</h1><p class="dq-sub">{{ $t('dedup_d_q.same_as_queue_duplicate_entity_resolutio') }}</p></div><ThemeToggle /></header>
    ...
    <template v-else-if="data">
      <div class="dq-stats"><PocketableChart chart-key="dedup_pending" chart="stat" :chart-props="{ value: data.pending, label: 'Pending Review', color: '#d97706' }" name="Pending Review" /><PocketableChart chart-key="dedup_reviewed" chart="stat" :chart-props="{ value: data.reviewed, label: 'Reviewed', color: '#16a34a' }" name="Reviewed" /><PocketableChart chart-key="dedup_total" chart="stat" :chart-props="{ value: data.total, label: 'Total SAME_AS' }" name="Total SAME_AS" /></div>
